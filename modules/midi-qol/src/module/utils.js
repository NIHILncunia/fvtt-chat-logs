...
export function createDamageDetail({ roll, item, versatile, defaultType = MQdefaultDamageType, ammo }) {
	...
	rollData.mod = 0;
	if (debugEnabled > 1)
		debug("CrreateDamageDetail: Passed roll is ", roll);
	if (debugEnabled > 1)
		debug("CrreateDamageDetail: Damage spec is ", parts);
	let partPos = 0;
	const validDamageTypes = Object.entries(getSystemCONFIG().damageTypes).deepFlatten().concat(Object.entries(getSystemCONFIG().healingTypes).deepFlatten());
	const allDamageTypeEntries = Object.entries(getSystemCONFIG().damageTypes).concat(Object.entries(getSystemCONFIG().healingTypes));
	...
	for (let part of parts) { // each spec,type is one of the damage lines
		...
		if (debugEnabled > 1)
			debug("CrreateDamageDetail: single Spec is ", spec, type, item);
...
export async function applyTokenDamageMany({ applyDamageDetails, theTargets, item, options = { hitTargets: new Set(), existingDamage: [], workflow: undefined, updateContext: undefined, forceApply: false, noConcentrationCheck: false } }) {
	...
	for (let t of theTargets) {
		...
		for (let i = 0; i < applyDamageDetails.length; i++) {
			...
			if (debugEnabled > 0)
				warn("applyTokenDamageMany | Damage Details plus resistance/save multiplier for ", targetActor.name, duplicate(damageDetail));
		}
...
export function checkRange(itemIn, tokenRef, targetsRef, showWarning = true) {
	...
	const checkRangeFunction = (item, token, targets) => {
		...
		if (item.system.range.units === "touch") {
			...
			if (getProperty(item, "system.properties.rch"))
				range += canvas?.dimensions?.distance ?? 5;
			longRange = 0;
		}
	...
	switch (configSettings.optionalRules.coverCalculation) {
		...
		case "tokencover":
			if (!installedModules.get("tokencover"))
				...
				if (coverValue < (safeGetGameSetting("tokencover", "cover-trigger-percent-low") ?? 0.5))
					coverBonus = 0;
				else if (coverValue < (safeGetGameSetting("tokencover", "cover-trigger-percent-medium") ?? 0.75))
					coverBonus = HALF_COVER;
				else if (coverValue < (safeGetGameSetting("tokencover", "cover-trigger-percent-high") ?? 1))
					coverBonus = THREE_QUARTERS_COVER;
				else
					coverBonus = FULL_COVER;
			}
...
export function hasCondition(tokenRef, condition) {
	...
	//@ts-expect-error
	const cub = game.cub;
	if (installedModules.get("condition-lab-triggler") && condition === "invisible" && cub.hasCondition("Invisible", [td.object], { warn: false }))
		return 1;
	if (installedModules.get("condition-lab-triggler") && condition === "hidden" && cub.hasCondition("Hidden", [td.object], { warn: false }))
		return 1;
	if (installedModules.get("dfreds-convenient-effects")) {
		//@ts-expect-error .dfreds
	...
	return new Promise((resolve, reject) => {
		...
		const callback = async (dialog, button) => {
			...
			if (button.value.trim().startsWith("ItemMacro") || button.value.trim().startsWith("Macro") || button.value.trim().startsWith("function")) {
				...
				if (result === undefined)
					console.warn(`midi-qol | bonusDialog | no way to call macro ${button.value}`);
			...
			if (macroToCall) {
				if (this instanceof Workflow) {
					...
					console.warn(`midi-qol | bonusDialog | no way to call macro ${macroToCall}`);
			}
			//@ts-expect-error D20Roll
...
export function evalCondition(condition, conditionData, errorReturn = true) {
	if (condition === undefined || condition === "")
		return true;
...
export function getConvenientEffectsReaction() {
	...
	if (reactionName) {
		...
		if (effect.flags?.statusId !== undefined)
			delete effect.flags.statusId;
		return effect;
	}
	return undefined;
}
export function getConvenientEffectsBonusAction() {
	...
	let result;
	if (bonusName)
		return dfreds.effects.all.find(ef => ef.name === bonusName);
	return undefined;
}
...
export async function computeFlankedStatus(target) {
	...
	while (allies.length > 1) {
		...
		for (let ally of allies) {
			...
			if (installedModules.get("dfreds-convenient-effects")) {
				//@ts-expect-error
				if (actor?.effects.some(ef => ef.name === game.dfreds.effects._incapacitated.name))
					continue;
			}
			if (checkRule("checkFlanking") === "ceflankedNoconga" && installedModules.get("dfreds-convenient-effects")) {
				//@ts-expect-error
...
export function isTargetable(target /*Token*/) {
	if (!target.actor)
		return false;
	if (getProperty(target.actor, "flags.midi-qol.neverTarget"))
		return false;
...
function displayContestedResults(chatCardId, resultContent, speaker, flavor) {
