import { warn, debug, log, i18n, MESSAGETYPES, error, MQdefaultDamageType, debugEnabled, MQItemMacroLabel, debugCallTiming, geti18nOptions, i18nFormat } from "../midi-qol.js";
...
export class Workflow {
	...
	setTemplateFlags(templateDoc, data, context, user) {
		...
		if (!getProperty(templateDoc, "flags.dnd5e.origin"))
			templateDoc.updateSource({ "flags.dnd5e.origin": this.item?.uuid });
		return true;
	}
	static async removeItemCardConfirmRollButton(itemCardId) {
		let chatMessage = game.messages?.get(itemCardId ?? "");
		if (!chatMessage)
			return;
	...
	static async removeItemCardAttackDamageButtons(itemCardId, removeAttackButtons = true, removeDamageButtons = true) {
		try {
			let chatMessage = game.messages?.get(itemCardId ?? "");
			if (!chatMessage)
				return;
	...
	*/
	...
	async WorkflowState_WaitForAttackRoll(context = {}) {
		...
		if (this.item.type === "tool") {
			...
			if (autoFastForwardAbilityRolls) {
				...
				const options = mergeObject(procOptions, { critical: this.item.criticalThreshold ?? 20, fumble: 1 });
	...
	async WorkflowState_AllRollsComplete(context = {}) {
		...
		if (configSettings.allowUseMacro && !configSettings.autoApplyDamage.includes("Card"))
	...
	async WorkflowState_Abort(context = {}) {
		...
		if (this.itemCardId)
			await game.messages?.get(this.itemCardId)?.delete();
		/*
			if (this.itemCardId) {
	...
	async checkAttackAdvantage() {
		...
		if (checkRule("nearbyFoe")
			...
			if (this.item.system.properties?.thr && actType === "rwak") {
				...
				//@ts-expect-error .first
				if (this.item.system.properties?.thr && getDistance(me, this.targets.first(), false) <= configSettings.optionalRules.nearbyFoe)
					nearbyFoe = false;
				else
					nearbyFoe = checkNearby(-1, canvas?.tokens?.get(this.tokenId), configSettings.optionalRules.nearbyFoe, { includeIncapacitated: false, canSee: true });
			}
	...
	async triggerTargetMacros(triggerList, targets = this.targets) {
		for (let target of targets) {
			...
			if (triggerList.includes("postTargetEffectApplication")) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("postTargetEffectApplication"), "TargetOnUse", "postTargetEffectApplication", { actor: target.actor, token: target });
			}
		...
		try {
			if (name.startsWith("function.")) {
				...
				if (name === MQItemMacroLabel) {
					...
					// ItemMacro.name
					if (!macroItem)
						macroItem = actorToUse.items.find(i => i.name === itemNameOrUuid && (getProperty(i.flags, "dae.macro") ?? getProperty(i.flags, "itemacro.macro")));
					if (!macroItem) {
	...
	*/
	...
	async checkHits() {
		...
		for (let targetToken of this.targets) {
			...
			let hitScale = 100;
			if (["scale", "scaleNoAR"].includes(checkRule("challengeModeArmor")) && !this.isCritical)
				hitScale = Math.floor(this.challengeModeScale[targetActor.uuid] * 100);
			let hitString;
