...
export async function doItemUse(wrapped, config = {}, options = {}) {
	...
	try {
		...
		if (autoCreatetemplate) {
			...
			if (workflow?.actor)
				setProperty(templateOptions, "flags.midi-qol.actorUuid", workflow.actor.uuid);
			if (workflow?.tokenId)
				setProperty(templateOptions, "flags.midi-qol.tokenId", workflow.tokenId);
			if (workflow)
				setProperty(templateOptions, "flags.midi-qol.workflowId", workflow.id);
			...
			if (this.item)
				setProperty(templateData, "flags.midi-qol.itemUuid", this.item.uuid);
			if (this.actor)
				setProperty(templateData, "flags.midi-qol.actorUuid", this.actor.uuid);
			if (!getProperty(templateData, "flags.dnd5e.origin"))
				setProperty(templateData, "flags.dnd5e.origin", this.item?.uuid);
			//@ts-expect-error
...
export async function doAttackRoll(wrapped, options = { versatile: false, resetAdvantage: false, chatMessage: undefined, createWorkflow: true, fastForward: false, advantage: false, disadvantage: false, dialogOptions: {}, isDummy: false }) {
	try {
		...
		if (workflow.suspended)
			workflow.unSuspend({ attackRoll: result });
...
export async function doDamageRoll(wrapped, { event = {}, systemCard = false, spellLevel = null, powerLevel = null, versatile = null, options = {} } = {}) {
	try {
		...
		needsMaxDamage = needsMaxDamage || ((targetMaxFlags.all && evalActivationCondition(workflow, targetMaxFlags.all, firstTarget))
		...
		needsMinDamage = needsMinDamage || ((targetMinFlags.damage && evalActivationCondition(workflow, targetMinFlags.all, firstTarget))
		...
		if (workflow.suspended)
			workflow.unSuspend({ damageRoll: result, otherDamageRoll: workflow.otherDamageRoll });
...
async function getChatData() {
	if (!this.system.activation?.condition)
	...
	try {
		...
		if (match)
			this.system.activation.condition = "...";
		...
		if (cond)
			this.system.activation.condition = cond;
	}
	return result;
}
...
export function selectTargets(templateDocument, data, user) {
	...
	if (user === game.user?.id && item)
		templateDocument.setFlag("midi-qol", "originUuid", item.uuid); // set a refernce back to the item that created the template.
	...
	if (workflow.needTemplate) {
		...
		if (workflow.suspended)
			workflow.unSuspend({ templateDocument });
...
export function shouldRollOtherDamage(workflow, conditionFlagWeapon, conditionFlagSpell) {
	...
	if (rollOtherDamage && conditionFlagToUse === "activation") {
		if ((workflow?.hitTargets.size ?? 0) === 0)
			return false;
