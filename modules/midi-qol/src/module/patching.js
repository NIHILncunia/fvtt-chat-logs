...
export async function bonusCheck(actor, result, category, detail) {
	if (!installedModules.get("betterrolls5e")) {
		...
		if (bonusFlags.length > 0) {
			...
			await bonusDialog.bind(data)(bonusFlags, detail ? `${category}.${detail}` : category, checkMechanic("displayBonusRolls"), `${actor.name} - ${title}`, "roll", "rollTotal", "rollHTML");
			result = data.roll;
		}
	}
	return result;
...
export async function removeConcentration(actor, deleteEffectUuid, options) {
	let result;
	try {
		if (debugEnabled > 0)
			warn("removeConcentration | ", actor?.name, deleteEffectUuid, options);
		const concentrationData = duplicate(actor.getFlag("midi-qol", "concentration-data") ?? {});
		...
		if (concentrationData?.targets && !options.concentrationEffectsDeleted) {
			if (deleteEffectUuid === undefined)
				options.concentrationDeleted = true; // concnetration effect will be picked up in the delete effects call
			...
			for (let target of concentrationData.targets) {
				...
				if (targetActor) {
					...
					if (effectsToDelete?.length > 0) {
						...
						if (debugEnabled > 0)
							warn("removeConcentration | removing effects", targetActor?.name, effectsToDelete, options);
		...
		if (!options.concentrationDeleted) {
			...
			if (concentrationEffect?.id && !options.concentrationDeleted && deleteEffectUuid !== concentrationEffect.uuid) {
				if (debugEnabled > 0)
					warn("removeConcentration | removing concentration effect", actor.name, concentrationEffect?.id, options);
				promises.push(actor?.deleteEmbeddedDocuments("ActiveEffect", [concentrationEffect.id], mergeObject(options, { concentrationDeleted: true, concentrationEffectsDeleted: true, noConcentrationCheck: true })));
			}
		}
